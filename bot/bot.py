...
"""Publish an unused entry as an image using Pandoc and WeasyPrint."""
...
WORDS_CACHE_PATH = BOT_DIR / "bot.cache"
...
def load_cached_ids(cache_path: Path) -> list[str]:
    """Load the IDs of entries that have already been published."""
    return json.loads(cache_path.read_text(encoding="utf-8"))


def load_resources(resource_dir: Path, cached_ids: set[str]) -> list[dict[str, str]]:
    """Load Markdown entries, omitting the entries already in the cache."""
    resources: list[dict[str, str]] = []
    current: dict[str, str] | None = None

    def add_current_resource() -> None:
        if current and current["id"] not in cached_ids:
...
def cache_published_entry(cache_path: Path, cached_ids: list[str], entry_id: str) -> None:
    """Persist an entry ID after its Mastodon post has succeeded."""
    if entry_id not in cached_ids:
        cached_ids.append(entry_id)
        cache_path.write_text(json.dumps(cached_ids), encoding="utf-8")
...
def main() -> None:
    ...
    cached_ids = load_cached_ids(WORDS_CACHE_PATH)
    resources = load_resources(RESOURCE_DIR, set(cached_ids))
    if not resources:
        sys.exit("No unused resources found.")
    ...
    cache_published_entry(WORDS_CACHE_PATH, cached_ids, entry["id"])
